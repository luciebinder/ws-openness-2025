...
```{dropdown} Challenge: Materials like slides or PDFs often lack clear navigation or modular structure.
...
```{dropdown} Challenge: Content is often locked behind institutional platforms (e.g. Moodle, ILIAS), making access difficult for external learners.  
...
```{dropdown} Challenge: Teaching often relies on static materials (e.g. PDFs, slides), which encourage passive consumption. 
...
```{dropdown} Challenge: Creating and improving teaching materials is often a solitary process.  
...
```{dropdown} Challenge: Educators may hesitate to adopt new tools due to complexity or lack of time.  
